...
function getDb() {
  try {
    ...
    console.error('[stripe-pause-mini] Firebase init error:', _e.message);
    throw _e;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[stripe-pause-mini] Paused mini subscription for uid:', uid);
    return res.status(200).json({ success: true });
  } catch (_e) {
    console.error('[stripe-pause-mini] Error:', _e.message);
