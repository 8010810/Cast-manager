...
function getDb() {
  try {
    ...
    console.error('[stripe-portal] Firebase init error:', _e.message);
    throw _e;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    var returnUrl = req.body.returnUrl || 'https://cast-manager.com';
    ...
    console.log('[stripe-portal] Created portal session for', roomId ? 'room:' + roomId : 'uid:' + uid);
    return res.status(200).json({ url: session.url });
  } catch (_e) {
    console.error('[stripe-portal] Error:', _e.message);
